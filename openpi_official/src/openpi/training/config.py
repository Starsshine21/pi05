...
class TrainConfig:
    ...
    # Optional assets namespace override. If set, norm stats are read from
    # `assets_base_dir / assets_namespace` instead of `assets_base_dir / name`.
    assets_namespace: str | None = None
    ...
    # PyTorch-only LoRA configuration. JAX training still uses freeze_filter/model variants.
    lora_config: lora_pytorch.LoRATrainingConfig | None = None

    @property
    def assets_dirs(self) -> pathlib.Path:
        """Get the assets directory for this config."""
        return (pathlib.Path(self.assets_base_dir) / (self.assets_namespace or self.name)).resolve()
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi05_pickplace_lora_pytorch",
        assets_namespace="pi05_pickplace",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_dim=32,
    ...
    TrainConfig(
        name="pi05_pickplace_full_pytorch",
        assets_namespace="pi05_pickplace",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_dim=32,
